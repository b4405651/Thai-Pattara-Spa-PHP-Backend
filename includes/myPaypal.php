...
public function __construct($config=""){
  ...
  //default settings
  $settings = array(
  //'business' => 'pratoom@naturalparkresort.com',         //paypal email address
  'business' => 'np-seller@gmail.com',         //paypal email address
  'currency' => 'RUB',                       //paypal currency
  'cursymbol'=> '฿',                   //currency symbol
  'location' => 'TH',                        //location code  (ex GB)
  'returnurl'=> '',//where to go back when the transaction is done.
  'returntxt'=> 'Return to Website',         //What is written on the return button in paypal
  ...
  $this->items = array();
    ...
    public function addSimpleItem($item){	
		//echo "INVOKE addSimpleItem<BR>";
		//Check the quantity and the name
		//echo (empty($item['quantity']) ? "TRUE" : "FALSE") . "<BR>";
		//echo (is_numeric($item['quantity']) ? "TRUE" : "FALSE") . "<BR>";
		//echo ($item['quantity'] > 0 ? "TRUE" : "FALSE") . "<BR>";
		//echo (empty($item['name']) ? "TRUE" : "FALSE") . "<BR>";
		...
		{ //And add the item to the cart if it is correct
			//echo "ITEM ADDED : " . $item['name'] . " " . $item['price'] . " " . $item['quantity'] . "<BR>";
			$items = $this->items;
			$items[] = $item;
			$this->items = $items;
		}
    ...
    public function addMultipleItems($items){
		//echo "INVOKE addMultipleItems [ITEM : " . count($items) . "]<BR>";
        if(!empty($items)){
			//echo "FOUND ITEM !!<BR>";
			foreach($items as $item){ //lopp through the items
				//echo "FOUND ITEM : " . $item['name'] . " " . $item['price'] . " " . $item['quantity'] . "<BR>";
		   ...
		   if(!empty($this->items)){foreach($this->items as $item){
			 $form.='
			  <div id="item_'.$cpt.'" class="itemwrap">
			   ...
			   <input type="hidden" name="shipping_'.$cpt.'" value="0" />
			 </div>';
			 $cpt++;
		   }}

		   //==> The submit button, (you can specify here your own button) <==//
		   $form.='<script>document.getElementById("paypal_checkout").submit();</script>';
		   $form.='</form>';
