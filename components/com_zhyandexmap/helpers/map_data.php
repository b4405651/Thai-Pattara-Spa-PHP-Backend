...
abstract class comZhYandexMapData
{
	...
	public static function getMarkerGroups($id, $placemarklistid, $explacemarklistid, $grouplistid, $categorylistid, $markergrouporder) 
	{
			...
			$query->select('h.*, c.title as category ')
				...
				->leftJoin('#__categories as c ON h.catid=c.id')
				->where(' EXISTS (SELECT 1 FROM #__zhyandexmaps_markers as m WHERE m.markergroup=h.id ' . $addWhereClause.')')
				;
			...
			if ((int)$markergroupctlmarker == 1)
			{
				if ((int)$markergroupctlpath != 0)
				{
					...
					$query->select('h.*, c.title as category ')
						...
						->leftJoin('#__categories as c ON h.catid=c.id')
						->where('( EXISTS (SELECT 1 FROM #__zhyandexmaps_markers as m WHERE m.markergroup=h.id ' . $addWhereClause. ')'.
						' or EXISTS (SELECT 1 FROM #__zhyandexmaps_paths as p WHERE p.markergroup=h.id ' . $addWhereClausePath.'))')
						;
				}
				else
				{
			...
			else
			{
				if ((int)$markergroupctlpath != 0)
				{
					...
					$query->select('h.*, c.title as category ')
						->from('#__zhyandexmaps_markergroups as h')
						->leftJoin('#__categories as c ON h.catid=c.id')
						->where('EXISTS (SELECT 1 FROM #__zhyandexmaps_paths as p WHERE p.markergroup=h.id ' . $addWhereClausePath.')')
						;
				}
				else
				{
	...
	public static function getMapAPIKey() 
	{
		...
		$mapapikey4map = $comparams->get( 'map_key');
